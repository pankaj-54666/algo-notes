...
class Solution {
    ...
    /* wrong approach. recursion factory is not taken care of & iterative solution is being written inside recursion*/
  ...
  cc      printf("[%d,%d]: (%d,%d,%d)\n",idx,state,val1,val2,val3);
    ...
    vector<int> C;
    /* passed*/
    /* i guess this should not work? as what about case taken from A,do not taken, taken from A
        in this case C be[3,INT_MIN,3]
    */
    int findAnsFour(int idx,int state,int last,vector<int>&A,vector<int>&B)
    {
        // if(idx-2>=0  && !(C[idx-2] <= C[idx-1])) return INF; //explain whats wrong
  ...
  cc      cout<<endl;
        ...
        int val1 = 0; //pick from A //extend the subarray on left when current elemtn is taken from A
        int val2 = 0; //pick from B //extend the subarray on left when current elemtn is taken from B
        int val3 = 0; //do not pick // Do not start at idx
